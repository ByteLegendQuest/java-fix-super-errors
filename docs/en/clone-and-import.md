...
`git clone https://github.com/ByteLegendQuest/java-fix-super-errors`

A project directory named `java-fix-super-errors` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-fix-super-errors` directory which is created just now.
